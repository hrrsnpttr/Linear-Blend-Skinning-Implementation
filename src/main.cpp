...
vector<glm::mat4> boneBindVec;
vector<vector<glm::mat4>> boneFrameVec;
...
void init()
{
	...
	cout << "> Z Key toggles bone view! <" << endl;
...
void render()
{
	...
	// Draw character
	double fps = 30;
	int frameCount = boneFrameVec.size(); // TODO: This number needs to be modified
	...
	for(const auto &shape : shapes) {
		...
		// Draw bones
		if(showBones) {
			for(int i = 0; i < boneFrameVec[frame].size(); i++) {
				// boneFrameVec[frame][bone#][Mat4 col][vec4 col]
				// There is likely a cleaner way to do this, sorry grader guy!
				glm::vec3 xdir = glm::vec3(boneFrameVec[frame][i][0][0], boneFrameVec[frame][i][0][1], boneFrameVec[frame][i][0][2]);
				glm::vec3 ydir = glm::vec3(boneFrameVec[frame][i][1][0], boneFrameVec[frame][i][1][1], boneFrameVec[frame][i][1][2]);
				glm::vec3 zdir = glm::vec3(boneFrameVec[frame][i][2][0], boneFrameVec[frame][i][2][1], boneFrameVec[frame][i][2][2]);
				glm::vec3 orig = glm::vec3(boneFrameVec[frame][i][3][0], boneFrameVec[frame][i][3][1], boneFrameVec[frame][i][3][2]);
				...
				glLineWidth(2);
				float sm = 4.0f; // Size Multiplier (sm) for bone frenet frame lines

				//draw x component
				glColor3f(1.0f, 0.0f, 0.0f);
				glBegin(GL_LINE_STRIP);
					glVertex3f(orig.x, orig.y, orig.z);
					glVertex3f(orig.x + sm * xdir.x, orig.y + sm * xdir.y, orig.z + sm * xdir.z);
				glEnd();
				...
				glBegin(GL_LINE_STRIP);
					glVertex3f(orig.x, orig.y, orig.z);
					glVertex3f(orig.x + sm * ydir.x, orig.y + sm * ydir.y, orig.z + sm * ydir.z);
				glEnd();
				...
				glBegin(GL_LINE_STRIP);
					glVertex3f(orig.x, orig.y, orig.z);
					glVertex3f(orig.x + sm * zdir.x, orig.y + sm * zdir.y, orig.z + sm * zdir.z);
				glEnd();
		...
		shape->update(frame, boneBindVec, boneFrameVec);
		shape->draw(frame);
		progSkin->unbind();
...
void loadSkeletonFile()
{
	...
	while(1) {
		...
		// create new bone vec
		vector<glm::mat4> tempBoneVec;

		// Parse lines
		string value;
		stringstream ss(line);

		// Handle frameCount boneCount line
		if(firstLines == 0) {
			firstLines = 1;
			ss >> value; // frameCount
			ss >> value; // boneCount
		...
		for(int i = 0; i < boneCount; i++) {
			...
			// Add to vec
			tempBoneVec.push_back(M);
		}

		if(firstLines == 1) {
			boneBindVec = tempBoneVec;
			firstLines = 2;
		} else {
			boneFrameVec.push_back(tempBoneVec);
		}
	}
	in.close();
	
	// Invert bind mats
	for(int i = 0; i < boneBindVec.size(); i++) {
		boneBindVec[i] = glm::inverse(boneBindVec[i]);
	}
}
...
int main(int argc, char **argv)
{
	...
	window = glfwCreateWindow(640, 480, "Harrison Potter - A2", NULL, NULL);
	if(!window) {
		glfwTerminate();
		return -1;
	}
